...
public abstract class jpeg_upsampler12_16
{
    /* Pointer to routine to do actual upsampling/conversion of one row group */
    public abstract void start_pass(jpeg_decompress_struct12_16 cinfo);

//    public abstract void upsample(jpeg_decompress_struct cinfo, byte[][][] input_buf, int[] input_buf_offset,
//                                  int[] in_row_group_ctr, int in_row_groups_avail,
//                                  byte[][] outputbuf, int[] out_row_counter, int out_rows_avail);
    public abstract void upsample(jpeg_decompress_struct12_16 cinfo, short[][][] input_buf, int[] input_buf_offset,
                                  int[] in_row_group_ctr, int in_row_groups_avail,
    ...
    //                 JSAMPARRAY output_buf));
//    public abstract void upmethod(jpeg_decompress_struct cinfo, byte[][][] input_buf, int[] in_row_group_ctr,
//                                  int in_row_groups_avail, byte[][] outputbuf, int[] out_row_counter,
//                                  int out_rows_avail); // no-op in jdsample
    public abstract void upmethod(jpeg_decompress_struct12_16 cinfo, short[][][] input_buf, int[] in_row_group_ctr,
                                  int in_row_groups_avail, short[][] outputbuf, int[] out_row_counter,
                                  int out_rows_avail); // no-op in jdsample

    public abstract void jinit_upsampler(jpeg_decompress_struct12_16 cinfo);
    ...
    public UPSAMPLE_METHODS[] methods = new UPSAMPLE_METHODS[jpeglib12_16.MAX_COMPONENTS];
     ...
     */
    public short[][][] color_buf = new short[jpeglib12_16.MAX_COMPONENTS][][];// JSAMPARRAY color_buf[MAX_COMPONENTS];
    public int[] color_buf_offset = new int[jpeglib12_16.MAX_COMPONENTS];

    public int next_row_out; /* counts rows emitted from color_buf */
    public int rows_to_go; /* counts rows remaining in image */

    /* Height of an input row group for each component. */
    public int[] rowgroup_height = new int[jpeglib12_16.MAX_COMPONENTS];

    /* These arrays save pixel expansion factors so that int_expand need not
     * recompute them each time.  They are unused for other upsampling methods.
     */
    public short[] h_expand = new short[jpeglib12_16.MAX_COMPONENTS];
    public short[] v_expand = new short[jpeglib12_16.MAX_COMPONENTS];
