...
public class jdphuff8 extends jdhuff8
{
    /* Pointers to derived tables (these workspaces have image lifespan) */
    public d_derived_tbl8[] derived_tbls = new d_derived_tbl8[jpeglib8.NUM_HUFF_TBLS];

    public d_derived_tbl8 ac_derived_tbl; /* active table during an AC scan */

    /*
     * Initialize for a Huffman-compressed scan.
     */

    public void start_pass_phuff_decoder(jpeg_decompress_struct8 cinfo)
    {
        ...
        else
        {
            ...
            if (cinfo.Ss > cinfo.Se || cinfo.Se >= jpeglib8.DCTSIZE2)
                bad = true;
        ...
        if (bad)
        {
            cinfo.err.ERREXIT4(JERR_BAD_PROGRESSION, cinfo.Ss, cinfo.Se, cinfo.Ah, cinfo.Al);
        }
        ...
        for (ci = 0; ci < cinfo.comps_in_scan; ci++)
        {
            ...
            {/* AC without prior DC scan */
                cinfo.err.WARNMS2(JWRN_BOGUS_PROGRESSION, cindex, 0);
            }
            ...
            for (coefi = cinfo.Ss; coefi <= cinfo.Se; coefi++)
            {
                ...
                if (cinfo.Ah != expected)
                    cinfo.err.WARNMS2(JWRN_BOGUS_PROGRESSION, cindex, coefi);
     ...
     */
    public boolean decode_mcu_DC_first(jpeg_decompress_struct8 cinfo, short[][] MCU_data)
    {
        int Al = cinfo.Al;
        int blkn, ci;
        short[] block;
        savable_state8 state = new savable_state8();
        d_derived_tbl8 tbl;
        jpeg_component_info8 compptr;
     ...
     * or first pass of successive approximation).
     */
    public boolean decode_mcu_AC_first(jpeg_decompress_struct8 cinfo, short[][] MCU_data)
    {
        int Se = cinfo.Se;
        int Al = cinfo.Al;
        int k;
            ...
            else
            {
                ...
                for (k = cinfo.Ss; k <= Se; k++)
                {
                    ...
                    if (s != 0)
                    {
                        ...
                        block[jutils8.jpeg_natural_order[k]] = (short) (s << Al);
                    }
                    else
                    {
                        if (r == 15)
     ...
     */
    public boolean decode_mcu_DC_refine(jpeg_decompress_struct8 cinfo, short[][] MCU_data)
    {
     ...
     */
    public boolean decode_mcu_AC_refine(jpeg_decompress_struct8 cinfo, short[][] MCU_data)
    {
        int Se = cinfo.Se;
        int p1 = 1 << cinfo.Al;  /* 1 in the bit position being coded */
        ...
        d_derived_tbl8 tbl;
        int num_newnz;
        int[] newnz_pos = new int[jpeglib8.DCTSIZE2];
                ...
                for (; k <= Se; k++)
                {
                    ...
                    if (s != 0)
                    {
                        if (s != 1)       /* size of new coef should always be 1 */
                            cinfo.err.WARNMS(JWRN_HUFF_BAD_CODE);
                    ...
                    {
                        thiscoef = block;
                        int thiscoef_offset = jutils8.jpeg_natural_order[k];
                    ...
                    if (s != 0)
                    {
                        int pos = jutils8.jpeg_natural_order[k];
     ...
     */
    public void jinit_phuff_decoder(jpeg_decompress_struct8 cinfo)
    {
        int[] coef_bit_ptr;
        int ci, i;

        /* Mark derived tables unallocated */
        for (i = 0; i < jpeglib8.NUM_HUFF_TBLS; i++)
        {
            derived_tbls[i] = null;
        }

        cinfo.coef_bits = new int[cinfo.num_components][jpeglib8.DCTSIZE2];
