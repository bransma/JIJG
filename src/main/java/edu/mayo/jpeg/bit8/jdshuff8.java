...
import static edu.mayo.jpeg.bit8.error8.ErrorStrings8.JWRN_NOT_SEQUENTIAL;
import static edu.mayo.jpeg.bit8.jpeglib8.DCTSIZE2;
...
public class jdshuff8 extends jdhuff8
{
    /* Pointers to derived tables (these workspaces have image lifespan) */
    public d_derived_tbl8[] dc_derived_tbls = new d_derived_tbl8[jpeglib8.NUM_HUFF_TBLS];
    public d_derived_tbl8[] ac_derived_tbls = new d_derived_tbl8[jpeglib8.NUM_HUFF_TBLS];

    /* Precalculated info set up by start_pass for use in decode_mcu: */

    /* Pointers to derived tables to be used for each block within an MCU */
    public d_derived_tbl8[] dc_cur_tbls = new d_derived_tbl8[jpeglib8.D_MAX_DATA_UNITS_IN_MCU];
    public d_derived_tbl8[] ac_cur_tbls = new d_derived_tbl8[jpeglib8.D_MAX_DATA_UNITS_IN_MCU];
    /* Whether we care about the DC and AC coefficient values for each block */
    public boolean[] dc_needed = new boolean[jpeglib8.D_MAX_DATA_UNITS_IN_MCU];
    public boolean[] ac_needed = new boolean[jpeglib8.D_MAX_DATA_UNITS_IN_MCU];

    public void start_pass_huff_decoder(jpeg_decompress_struct8 cinfo)
    {
        ...
        if (cinfo.Ss != 0 || cinfo.Se != DCTSIZE2 - 1 ||
            ...
            cinfo.err.WARNMS(JWRN_NOT_SEQUENTIAL);
    ...
    public void jinit_shuff_decoder(jpeg_decompress_struct8 cinfo)
    {
        entropy_decode_mcu = ENTROPY_DECODE_MCU.decode_mcu;

        /* Mark tables unallocated */
        for (int i = 0; i < jpeglib8.NUM_HUFF_TBLS; i++)
        {
    ...
    public boolean decode_mcu(jpeg_decompress_struct8 cinfo, short[][] MCU_data)
    {
        int blkn;
        ...
        if (!insufficient_data)
        {
            ...
            for (blkn = 0; blkn < cinfo.data_units_in_MCU; blkn++)
            {
                ...
                d_derived_tbl8 dctbl = dc_cur_tbls[blkn];
                d_derived_tbl8 actbl = ac_cur_tbls[blkn];
                int k, r;
                ...
                if (ac_needed[blkn])
                {
                    ...
                    for (k = 1; k < DCTSIZE2; k++)
                    {
                        ...
                        if (s != 0)
                        {
                             ...
                             */
                            block[jutils8.jpeg_natural_order[k]] = (short) s;
                        }
                        else
                        {
                            if (r != 15)
