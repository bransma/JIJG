...
import static edu.mayo.jpeg.bit8.error8.ErrorStrings8.JERR_BAD_RESTART;
...
public class jddiffct8 extends d_coef_controller8
{
    ...
    int[][][] diff_buf = new int[jmorecfg8.MAX_COMPONENTS][][];  /* iMCU row of differences */
    int[][][] undiff_buf = new int[jmorecfg8.MAX_COMPONENTS][][]; /* iMCU row of undiff'd samples */

    /* In multi-pass modes, we need a virtual sample array for each component. */
    jvirt_sarray_control8[] whole_image = new jvirt_sarray_control8[jmorecfg8.MAX_COMPONENTS];

    public void start_iMCU_row(jpeg_decompress_struct8 cinfo)
        /* Reset within-iMCU-row counters for a new row (input side) */
    {
        jpeg_d_codec8 losslsd = cinfo.codec;
    ...
    public void start_input_pass(jpeg_decompress_struct8 cinfo)
    {
        ...
        if (cinfo.restart_interval % cinfo.MCUs_per_row != 0)
            cinfo.err.ERREXIT2(JERR_BAD_RESTART, cinfo.restart_interval, cinfo.MCUs_per_row);
     ...
     */
    public int decompress_data(jpeg_decompress_struct8 cinfo, byte[][][] output_buf, int[] buffer_offset)
    {
        jpeg_d_codec8 losslsd = cinfo.codec;
    ...
    public int consume_data(jpeg_decompress_struct8 cinfo)
    {
        ...
        byte[][][] buffer = new byte[jpeglib8.MAX_COMPS_IN_SCAN][][];
    ...
    public int output_data(jpeg_decompress_struct8 cinfo, byte[][][] output_buf, int[] buffer_offset)
    {
        int last_iMCU_row = cinfo.total_iMCU_rows - 1;
        int ci, samp_rows, row;
        byte[][] buffer;
        jpeg_component_info8 compptr = cinfo.comp_info[0];
     ...
     */
    public void jinit_d_diff_controller(jpeg_decompress_struct8 cinfo, boolean need_full_buffer)
    {
        int ci;
        jpeg_component_info8 compptr = cinfo.comp_info[0];
        int compptr_offset = 0;
        ...
        for (ci = 0; ci < cinfo.num_components; ci++)
        {
            width = (int) jutils8.jround_up(compptr.width_in_data_units, compptr.h_samp_factor);
        ...
        if (need_full_buffer)
        {
            ...
            {
                width = (int) jutils8.jround_up(compptr.width_in_data_units, compptr.h_samp_factor);
                height = (int) jutils8.jround_up(compptr.height_in_data_units, compptr.v_samp_factor);
