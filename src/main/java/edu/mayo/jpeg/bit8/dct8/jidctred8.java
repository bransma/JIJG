...
public class jidctred8 extends jdct8
{
    public final int CONST_BITS = 13;
    public final int PASS1_BITS = 2;
     ...
     */
    public void jpeg_idct_4x4(jpeg_decompress_struct8 cinfo, jpeg_component_info8 compptr, short[] coef_block,
                              byte[][] output_buf, int output_buff_offset, int output_col)
    {
        int tmp0, tmp2, tmp10, tmp12;
        int z1, z2, z3, z4;

        // JCOEFPTR = pointer to JCOEF = pointer to short = short array //
        short[] inptr;
        // ISLOW_MULT_TYPE *: typedef MULTIPLIER ISLOW_MULT_TYPE; #define MULTIPLIER  int
        int[] quantptr;
        int[] wsptr;
        // JSAMPROW = pointer to JSAMP = pointer to byte = byte array //
        byte[] outptr;
        ...
        for (ctr = 0; ctr < 4; ctr++)
        {
            ...
            {
                /* AC terms all zero */
                byte dcval = range_limit[range_limit_offset + DESCALE(wsptr[0], PASS1_BITS + 3)
                        & RANGE_MASK];
     ...
     */
    public void jpeg_idct_2x2(jpeg_decompress_struct8 cinfo, jpeg_component_info8 compptr, short[] coef_block,
                              byte[][] output_buf, int output_buff_offset, int output_col)
    {
        int tmp0, tmp10, z1;
        short[] inptr;
        ...
        for (ctr = 0; ctr < 2; ctr++)
        {
            ...
            {
                /* AC terms all zero */
                byte dcval = range_limit[DESCALE(wsptr[wsptr_offset], PASS1_BITS + 3) & RANGE_MASK];
     ...
     */
    public void jpeg_idct_1x1(jpeg_decompress_struct8 cinfo, jpeg_component_info8 compptr, short[] coef_block,
                              byte[][] output_buf, int output_buff_offset, int output_col)
    {
        int dcval;
        int[] quantptr;
