...
public class jdmaster8 extends jpeg_decomp_master8
{
    public boolean use_merged_upsample(jpeg_decompress_struct8 cinfo)
    {
        if (jmorecfg8.UPSAMPLE_SCALING_SUPPORTED)
        {
            ...
            if (cinfo.jpeg_color_space != jpeglib8.J_COLOR_SPACE.JCS_YCbCr || cinfo.num_components != 3
    ...
    public void jpeg_calc_output_dimensions(jpeg_decompress_struct8 cinfo)
     ...
     */
    public void prepare_range_limit_table(jpeg_decompress_struct8 cinfo)
    /* Allocate and fill in the sample_range_limit table */
    {
        byte[] table = new byte[(5 * (jmorecfg8.MAXJSAMPLE + 1) + jmorecfg8.CENTERJSAMPLE)];// * SIZEOF(JSAMPLE)]//JSAMPLE * table;
        int i;
        ...
        int offset = jmorecfg8.MAXJSAMPLE + 1;
        cinfo.sample_range_limit_offset = offset;
        cinfo.sample_range_limit = table;

        /* Main part of "simple" table: limit[x] = x */
        for (i = 0; i <= jmorecfg8.MAXJSAMPLE; i++)
        {
            table[i + offset] = (byte) i;
        }

        offset += jmorecfg8.CENTERJSAMPLE; /* Point to where post-IDCT table starts */

        /* End of simple table, rest of first half of post-IDCT table */
        for (i = jmorecfg8.CENTERJSAMPLE; i < 2 * (jmorecfg8.MAXJSAMPLE + 1); i++)
            table[i + offset] = (byte) jmorecfg8.MAXJSAMPLE;

        /* Second half of post-IDCT table */

        System.arraycopy(cinfo.sample_range_limit, cinfo.sample_range_limit_offset, table,
                (4 * (jmorecfg8.MAXJSAMPLE + 1) - jmorecfg8.CENTERJSAMPLE), jmorecfg8.CENTERJSAMPLE);
     ...
     * settings.
     */
    public void master_selection(jpeg_decompress_struct8 cinfo)
    {
        ...
        if (cinfo.quantize_colors)
        {
            ...
            {
               cinfo.err.ERREXIT(JERR_NOTIMPL);
            }
            ...
            {
                if (jmorecfg8.QUANT_1PASS_SUPPORTED)
                {
                    quantizer_1pass = new jquant1_8();
        ...
        if (!cinfo.raw_data_out)
        {
            ...
            {
                if (jmorecfg8.UPSAMPLE_MERGING_SUPPORTED)
                {
                    jdmerge8 merged_upsample = new jdmerge8();
        ...
        {
            jpeg_d_main_controller8 main = new jdmainct8();
            cinfo.main = main;
    ...
    public void prepare_for_output_pass(jpeg_decompress_struct8 cinfo)
    {
        if (is_dummy_pass)
        {
            if (jmorecfg8.QUANT_2PASS_SUPPORTED)
            {
                ...
                cinfo.post.start_pass(cinfo, jpegint8.J_BUF_MODE.JBUF_CRANK_DEST);
                cinfo.main.start_pass(cinfo, jpegint8.J_BUF_MODE.JBUF_CRANK_DEST);
            }
            else
            {
                cinfo.err.ERREXIT(JERR_NOT_COMPILED);
            }
        }
        else
        {
            if (cinfo.quantize_colors && cinfo.colormap == null)
            {
                ...
                {
                    cinfo.err.ERREXIT(JERR_MODE_CHANGE);
                }
            }
            ...
            if (!cinfo.raw_data_out)
            {
                ...
                cinfo.post.start_pass(cinfo,
                        (is_dummy_pass ? jpegint8.J_BUF_MODE.JBUF_SAVE_AND_PASS : jpegint8.J_BUF_MODE.JBUF_PASS_THRU));
                cinfo.main.start_pass(cinfo, jpegint8.J_BUF_MODE.JBUF_PASS_THRU);
            }
        }
    ...
    public void finish_output_pass(jpeg_decompress_struct8 cinfo)
    {
        if (cinfo.quantize_colors)
        {
    ...
    public void jpeg_new_colormap(jpeg_decompress_struct8 cinfo)
    {
     ...
     */
    public void jinit_master_decompress(jpeg_decompress_struct8 cinfo)
    {
