...
import bransford.jpeg.bit8.structs8.jpeg_d_codec8;
...
public class jdlossls8 extends jpeg_d_codec8
{
    public jddiffct8 diff = new jddiffct8();

    /* Huffman entropy module */
    jdlhuff8 entropy_private = new jdlhuff8();

    jdscaler8 scaler_private = new jdscaler8();

    jdpred8 pred = new jdpred8();

    /* It is useful to allow each component to have a separate undiff method. */
    public jdpred8.predict_undifference[] predict_undifference =
            new jdpred8.predict_undifference[jmorecfg8.MAX_COMPONENTS];
    ...
    public void scaler_scale(jpeg_decompress_struct8 cinfo, int[] diff_buf, byte[] output_buf, int width)
    {
        ...
        {
            case noscale -> scaler_private.noscale(cinfo, diff_buf, output_buf, width);
            case simple_downscale -> scaler_private.simple_downscale(cinfo, diff_buf, output_buf, width);
            case simple_upscale -> scaler_private.simple_upscale(cinfo, diff_buf, output_buf, width);
        }
    }
    ...
    public int entropy_decode_mcus(jpeg_decompress_struct8 cinfo,
                                   int[][][] diff_buf,
                                   int MCU_row_num,
                                   int MCU_col_num,
                                   int nMCU)
    ...
    {
        jdpred8.predict_undifference predict_method = predict_undifference[comp_index];
    ...
    public void jinit_lossless_d_codec(jpeg_decompress_struct8 cinfo)
    {
