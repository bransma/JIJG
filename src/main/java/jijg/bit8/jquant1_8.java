...
public class jquant1_8 extends jpeg_color_quantizer8
{
    ...
    public byte[][] colorindex; //JSAMPARRAY colorindex;    /* Precomputed mapping for speed */
    ...
    public int select_ncolors(jpeg_decompress_struct8 cinfo, int[] Ncolors)
        ...
        /* Return value is total number of colors (product of Ncolors[] values). */
    {
        ...
        boolean changed;
        long temp;
        int[] RGB_order = {jmorecfg8.RGB_GREEN, jmorecfg8.RGB_RED, jmorecfg8.RGB_BLUE};
        ...
        if (iroot < 2)
            cinfo.err.ERREXIT1(ErrorStrings8.JERR_QUANT_FEW_COLORS, temp);
        ...
        do
        {
            ...
            for (i = 0; i < nc; i++)
            {
                j = (cinfo.out_color_space == jpeglib8.J_COLOR_SPACE.JCS_RGB ? RGB_order[i] : i);
    ...
    public int output_value(jpeg_decompress_struct8 cinfo, int ci, int j, int maxj)
        ...
        /* The output values must fall in 0..jmorecfg.MAXJSAMPLE in increasing order */
    {
         ...
         */
        return ((j * jmorecfg8.MAXJSAMPLE + maxj / 2) / maxj);
    }

    public int largest_input_value(jpeg_decompress_struct8 cinfo, int ci, int j, int maxj)
        /* Return largest input value that should map to j'th output value */
        /* Must have largest(j=0) >= 0, and largest(j=maxj) >= jmorecfg.MAXJSAMPLE */
    {
        /* Breakpoints are halfway between values returned by output_value */
        return (((2 * j + 1) * jmorecfg8.MAXJSAMPLE + maxj) / (2 * maxj));
    }

    /*
     * Create the colormap.
     */
    public void create_colormap(jpeg_decompress_struct8 cinfo)
    {
        //my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo.cquantize;
        byte[][] colormap; /* Created colormap */
    ...
    public void create_colorindex(jpeg_decompress_struct8 cinfo)
    {
         ...
         */
        if (cinfo.dither_mode == jpeglib8.J_DITHER_MODE.JDITHER_ORDERED)
        {
            pad = jmorecfg8.MAXJSAMPLE * 2;
            is_padded = true;
        }
        else
        {
        ...
        colorindex = cinfo.mem.alloc_sarray(cinfo, (jmorecfg8.MAXJSAMPLE + 1 + pad), cinfo.out_color_components);
        ...
        for (i = 0; i < cinfo.out_color_components; i++)
        {
            ...
            for (j = 0; j <= jmorecfg8.MAXJSAMPLE; j++)
            {
                ...
                colorindex[i][j] = (byte) (val * blksize);
            }
            /* Pad at both ends if necessary */
            if (!is_padded)
            {
                // TODO - this is not correct
                for (j = 1; j <= jmorecfg8.MAXJSAMPLE; j++)
                {
                    colorindex[i][j] = colorindex[i][0];
                    colorindex[i][jmorecfg8.MAXJSAMPLE + j] = colorindex[i][jmorecfg8.MAXJSAMPLE];
                }
            }
        }
    }
    ...
    public int[][] make_odither_array(jpeg_decompress_struct8 cinfo, int ncolors)
    {
        int j, k;
        long num, den;
        ...
        for (j = 0; j < ODITHER_SIZE; j++)
        {
            ...
            {
                num = ((long) (ODITHER_CELLS - 1 - 2L * base_dither_matrix[j][k])) * jmorecfg8.MAXJSAMPLE;
     ...
     */
    public void create_odither_tables(jpeg_decompress_struct8 cinfo)
    {
        int[] odither_ptr;
        int i, j, nci;
    ...
    public void color_quantize(jpeg_decompress_struct8 cinfo, byte[][] input_buf, byte[][] output_buf, int num_rows)
    {
        ...
        {
            case color_quantize:
                color_quantize_impl(cinfo, input_buf, output_buf, num_rows);
                break;
            case color_quantize3:
                color_quantize3(cinfo, input_buf, output_buf, num_rows);
                break;
            case quantize_ord_dither:
                quantize_ord_dither(cinfo, input_buf, output_buf, num_rows);
                break;
            case quantize3_ord_dither:
                quantize3_ord_dither(cinfo, input_buf, output_buf, num_rows);
                break;
            case quantize_fs_dither:
                quantize_fs_dither(cinfo, input_buf, output_buf, num_rows);
                break;
        }
    }

    public void color_quantize_impl(jpeg_decompress_struct8 cinfo, byte[][] input_buf, byte[][] output_buf,
                                    int num_rows)
    {
    ...
    public void color_quantize3(jpeg_decompress_struct8 cinfo, byte[][] input_buf, byte[][] output_buf, int num_rows)
        /* Fast path for out_color_components==3, no dithering */
    {

    }

    public void quantize_ord_dither(jpeg_decompress_struct8 cinfo, byte[][] input_buf, byte[][] output_buf, int num_rows)
        /* General case, with ordered dithering */
    {

    }

    public void quantize3_ord_dither(jpeg_decompress_struct8 cinfo, byte[][] input_buf, byte[][] output_buf, int num_rows)
    ...
    public void quantize_fs_dither(jpeg_decompress_struct8 cinfo, byte[][] input_buf, byte[][] output_buf, int num_rows)
     ...
     */
    public void alloc_fs_workspace(jpeg_decompress_struct8 cinfo)
    {
     ...
     */
    public void start_pass_1_quant(jpeg_decompress_struct8 cinfo, boolean is_pre_scan)
    {
        int arraysize;
        int i;
     ...
     */
    public void finish_pass_1_quant(jpeg_decompress_struct8 cinfo)
    {
     ...
     */
    public void new_color_map_1_quant(jpeg_decompress_struct8 cinfo)
    {
     ...
     */
    public void jinit_1pass_quantizer(jpeg_decompress_struct8 cinfo)
    {
