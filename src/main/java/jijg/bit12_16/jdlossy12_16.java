...
public class jdlossy12_16 extends jpeg_d_codec12_16
{
    ...
    public int decompress_data(jpeg_decompress_struct12_16 cinfo, short[][][] output_buf, int[] output_buffer_offset)
    {
        int ret = 0;
        //  passthrough  dcoef
        switch (decompress_data)
        {
            case decompress_data:
                ret = coef_private.decompress_data(cinfo, output_buf, output_buffer_offset);
                break;
            case decompress_onepass:
                ret =  coef_private.decompress_onepass(cinfo, output_buf, output_buffer_offset);
                break;
            case decompress_smooth_data:
                ret = coef_private.decompress_smooth_data(cinfo, output_buf, output_buffer_offset);
                break;
        }
