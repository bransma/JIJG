...
public class jdcoefct12_16 extends d_coef_controller12_16
{
    ...
    public void start_output_pass(jpeg_decompress_struct12_16 cinfo)
    {
        ...
        if (jmorecfg12_16.BLOCK_SMOOTHING_SUPPORTED)
        {
            /* If multipass, check to see whether to use block smoothing on this pass */
            if (lossyd.coef_arrays != null)
            {
                if (cinfo.do_block_smoothing && smoothing_ok(cinfo))
                    lossyd.decompress_data = jpeg_d_codec12_16.DECOMPRESS.decompress_smooth_data;
                else
                    lossyd.decompress_data = jpeg_d_codec12_16.DECOMPRESS.decompress_data;
            }
        }
    ...
    // public int decompress_onepass(jpeg_decompress_struct cinfo, byte[][][] output_buf, int[] buffer_offset)
    public int decompress_onepass(jpeg_decompress_struct12_16 cinfo, short[][][] output_buf, int[] buffer_offset)
    {
        jdlossy12_16 lossyd = (jdlossy12_16) cinfo.codec;
        int MCU_col_num; /* index of current MCU within row */
    ...
    // public int decompress_data(jpeg_decompress_struct cinfo, byte[][][] output_buf, int[] output_buffer_offset)
    ...
    public boolean smoothing_ok(jpeg_decompress_struct12_16 cinfo)
    {
        boolean smoothing_useful = false;
        int ci, coefi;
        jpeg_component_info12_16 compptr;
        JQUANT_TBL12_16 qtable;
        int[] coef_bits;

        if (!(cinfo.process == jpeglib12_16.J_CODEC_PROCESS.JPROC_PROGRESSIVE || cinfo.coef_bits == null))
        {
            return false;
        }
    ...
    // public int decompress_smooth_data(jpeg_decompress_struct cinfo, byte[][][] output_buf, int[] output_buffer_offset)
    public int decompress_smooth_data(jpeg_decompress_struct12_16 cinfo, short[][][] output_buf, int[] output_buffer_offset)
    {
        jdlossy12_16 lossyd = (jdlossy12_16) cinfo.codec;
        int last_iMCU_row = cinfo.total_iMCU_rows - 1;
        ...
        short[] workspace = new short[jpeglib12_16.DCTSIZE2];
        int[] coef_bits;
        JQUANT_TBL12_16 quanttbl;
     ...
     */
    public void jinit_d_coef_controller(jpeg_decompress_struct12_16 cinfo, boolean need_full_buffer)
    {
        jdlossy12_16 lossyd = (jdlossy12_16) cinfo.codec;
            ...
            if (jmorecfg12_16.D_MULTISCAN_FILES_SUPPORTED)
            {
                ...
                for (ci = 0; ci < cinfo.num_components; ci++)
                {
                    ...
                    whole_image[ci] = cinfo.mem.request_virt_barray(cinfo, true,
                            (int) jutils12_16.jround_up(compptr.width_in_data_units, compptr.h_samp_factor),
                            (int) jutils12_16.jround_up(compptr.height_in_data_units, compptr.v_samp_factor),
                            access_rows);
                }
                lossyd.consume_data = jpeg_d_codec12_16.CONSUME_DATA.consume_data;
                lossyd.decompress_data = jpeg_d_codec12_16.DECOMPRESS.decompress_data;
        ...
        else
        {
            ...
            MCU_buffer = new short[jpeglib12_16.D_MAX_DATA_UNITS_IN_MCU][jpeglib12_16.DCTSIZE2];
            lossyd.consume_data = jpeg_d_codec12_16.CONSUME_DATA.dummy_consume_data;
            lossyd.decompress_data = jpeg_d_codec12_16.DECOMPRESS.decompress_onepass;
